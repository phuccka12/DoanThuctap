...
// \u2500\u2500\u2500 Node type meta \u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500
const NODE_META = {
  reading:    { label: '\u0110\u1ECDc hi\u1EC3u',   icon: '\u{1F4D6}', color: 'text-blue-400',    bg: 'bg-blue-500/15 border-blue-400/30'     },
  listening:  { label: 'Nghe',       icon: '\u{1F3A7}', color: 'text-purple-400',  bg: 'bg-purple-500/15 border-purple-400/30' },
  vocabulary: { label: 'T\u1EEB v\u1EF1ng',    icon: '\u{1F4DD}', color: 'text-emerald-400', bg: 'bg-emerald-500/15 border-emerald-400/30'},
  quiz:       { label: 'Quiz',       icon: '\u{1F9E9}', color: 'text-orange-400',  bg: 'bg-orange-500/15 border-orange-400/30' },
  writing:    { label: 'Vi\u1EBFt',       icon: '\u270D\uFE0F',  color: 'text-yellow-400',  bg: 'bg-yellow-500/15 border-yellow-400/30' },
  speaking:   { label: 'N\u00F3i',        icon: '\u{1F399}\uFE0F', color: 'text-rose-400',    bg: 'bg-rose-500/15 border-rose-400/30'     },
  video:      { label: 'Video',      icon: '\u{1F3AC}', color: 'text-sky-400',     bg: 'bg-sky-500/15 border-sky-400/30'       },
};
...
function getNodeMeta(type = '') {
  ...
  return { label: type || 'H\u1ECDc', icon: '\u{1F4DA}', color: 'text-gray-400', bg: 'bg-gray-500/10 border-gray-400/20' };
}
  ...
  if (!lesson || nodes.length === 0) {
    return (
      <div className={cn('min-h-screen flex items-center justify-center text-center px-6', t.page)}>
        <div>
          <div className="text-6xl mb-4">\u{1F636}</div>
    ...
    <div className={cn('min-h-screen flex flex-col', t.page)}>
          ...
          <button
            onClick={() => {
              if (completedNodes.size === 0 || window.confirm('Tho\u00E1t? Ti\u1EBFn \u0111\u1ED9 ch\u01B0a \u0111\u01B0\u1EE3c l\u01B0u.'))
                navigate(topicId ? \`/learn/topics/\${topicId}\` : '/learn');
            }}
            className={cn(
      ...
      {showModal && (
        <CompletionModal
          ...
          onClose={() => navigate(topicId ? \`/learn/topics/\${topicId}\` : '/learn')}
          onNextLesson={() => nextLesson && navigate(\`/learn/lessons/\${nextLesson._id}\`)}
        />
      )}
    </div>
  );
...
const filePath = path.join(__dirname, 'client-web', 'src', 'pages', 'LessonPlayer.jsx');
